...
function onGettingFile(f, item) {
    ...
    if (file.size >= 100000000) {
        document.querySelector('#btn-upload-dropdown').style.visibility = 'hidden';
        fsize.innerHTML = 'UPLOAD DISABLED > 100MB: ' + bytesToSize(file.size);
    }
    console.log(fsize);
    ...
    if (item.php) {
        //html = 'Click to download file from: <a href="' + item.php + '" target="_blank">' + item.php;
    ...
    browserCache.querySelector('.cross-icon').style.visibility = "hidden";
...
document.querySelector('#btn-recordings-list').onclick = function(e) {
    if (window.confirm('Do you want to delete this video and close the window?')) {
        window.close();
    }
};
/*
